...
public enum ServiceType {
  ...
  PROCESS("process"),
  CATALOG("catalog"),
  CATALOG_WEB("catalog_web"),
  CITATION("citation"),
  ARCHIVE("archive"),
  REQUEST("request"),
  WF_HANDLER("wf_handler"),
  MONITORING("monitoring"),
  EGA_BEACON("ega_beacon"),
  SUBMITTER_PORTAL("submitter_portal"),
  REQUESTER_PORTAL("requester_portal"),
  ...
  public static ServiceType parse(String value) {
    ...
    if (StringUtils.equalsIgnoreCase(value, CONFIG.value)) {
      ...
      type = CATALOG;
    } else if (StringUtils.equalsIgnoreCase(value, CATALOG_WEB.value)) {
      type = CATALOG_WEB;
    } else if (StringUtils.equalsIgnoreCase(value, CITATION.value)) {
      type = CITATION;
