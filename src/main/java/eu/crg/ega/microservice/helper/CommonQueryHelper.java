...
public class CommonQueryHelper {
  ...
  public static MultiValueMap<String, String> toMap(CommonQuery commonQuery) {
    ...
    if (commonQuery.getSort() != null) {
      map.add(ParamName.SORT, commonQuery.getSort().toString());
    }
    return map;
  }
  ...
  public static CommonQuery parseQuery(Map<String, String> params) {
    ...
    if (StringUtils.isNotBlank(sort)) {
      String[] sortTokens = StringUtils.split(sort, ":");
      String column = StringUtils.trim(sortTokens[0]);
      String ordering = StringUtils.trim(sortTokens[1]);
      Sort parsedSort =
          new Sort(new Sort.Order(
              StringUtils.equalsIgnoreCase(ordering, Direction.ASC.toString()) ? Direction.ASC
                  : Direction.DESC, column));
      commonQuery.setSort(parsedSort);
    }
    
    if (log.getLevel() == Level.DEBUG) {
      log.debug("limit = " + commonQuery.getLimit());
      log.debug("skip = " + commonQuery.getSkip());
      log.debug("sort = " + commonQuery.getSort().toString());
