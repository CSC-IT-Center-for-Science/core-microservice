...
import eu.crg.ega.microservice.dto.auth.DrupalRequesterUser;
import eu.crg.ega.microservice.dto.auth.MongoInternalUser;
import eu.crg.ega.microservice.dto.auth.PostgresSubmitterUser;
import eu.crg.ega.microservice.dto.auth.User;
import eu.crg.ega.microservice.enums.LoginType;
import eu.crg.ega.microservice.enums.UserRole;
...
//                MongoSubmitterUser.builder().egaBox("ega-box-" + userId).alias("CRG_test").build())
  ...
  public static void addAnonymousUserToContext() {
    GrantedAuthority authority = new SimpleGrantedAuthority("ROLE_ANONYMOUS");
    SecurityContextHolder.getContext().setAuthentication(
        new AnonymousAuthenticationToken("anonymous", new User(), Stream.of(authority).collect(Collectors.toList())));
  }
